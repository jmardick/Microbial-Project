...
## Target m/z values of interest for ratio plots
...
target_mz <- c(174.0884, 688.4923, 245.0433, 746.4519, 325.114, 702.5079, 730.5392, 509.3484, 728.5236, 749.5248, 242.155, 735.5182, 253.2173, 325.114, 270.1863, 358.216, 662.4766, 765.53, 714.5079, 925.5677)
```
...
matched_mz <- sapply(target_mz, function(x) {
  colnames(xall)[which.min(abs(as.numeric(colnames(xall)) - x))]
})
target_data <- xall[, matched_mz]

## Check mass error of selected peaks
target_data_mass_error <- abs(as.numeric(colnames(target_data)) - target_mz)
print(target_data_mass_error)

## Rename using actual peak name
colnames(target_data) <- target_mz
target_data <- as.matrix(target_data)
```

``` {r ratios}
## Select ratio peaks (From "Microbial-Project\outputs\ratio-lasso_gram_featurelist_2025-06-23\2025-06-23_16.12_ratio-lasso_gram_featurelist.html")
numerator <- target_data[, c("174.0884", "245.0433", "325.114", "325.114", "509.3484", "509.3484", "242.155", "253.2173", "270.1863", "662.4766", "714.5079")]
denominator <- target_data[, c("688.4923", "746.4519", "702.5079", "730.5392", "728.5236", "749.5248", "735.5182", "325.114", "358.216", "765.53", "925.5677")]

## Calculate ratios
ratio_df <- purrr::map_dfr(
  seq_along(numerator),
  function(i) {
    num_name <- colnames(numerator)[i]
    denom_name <- colnames(denominator)[i]
    data.frame(
      class = yall,
      ratio = numerator[[i]] / denominator[[i]],
      ratio_label = paste0("m/z ", num_name, " / ", denom_name),
...
## Remove ratios not from feature list
ratio_df <- ratio_df |>
  filter(ratio_label != "m/z NA / NA")
```

``` {r plots}
## Split into a list of one-ratio data frames
ratio_list <- split(ratio_df, ratio_df$ratio_label)

## Faceted boxplot by ratio
plot_list <- imap(ratio_list, function(df, ratio_nm) {
...
# If you want to inspect the first plot in the RStudio viewer:
print(plot_list[[12]])

## Save plots
imap(plot_list, function(p, ratio_nm) {
  
  # make a file‑safe name, e.g. "m_z_174.0884___688.4923.png"
  safe_nm <- paste0(
    "ratio_", 
    gsub("[^0-9A-Za-z]+", "_", ratio_nm),
    ".png"
  )
  
  # full path in your files_dir
  out_path <- file.path(files_dir, safe_nm)
  
  # save with your preferred dimensions & dpi
  ggsave(
    filename = out_path,
    plot     = p,
    width    = 8,
...
``` {r binned class plots}
## Faceted boxplot by ratio
binned_plot <- ggplot(ratio_df, aes(x = binned_class, y = ratio)) +
  geom_boxplot(outlier.shape = NA, fill = "lightblue", alpha = 0.6) +
  geom_jitter(width = 0.2, size = 0.8, alpha = 0.4) +
  facet_wrap(~ ratio_label, scales = "free_y", ncol = 1) +
  labs(
    title = "Ratios of Selected Peaks By Tumor Percentage",
    x = "Tumor Percentage (%)",
    y = "Intensity Ratio"
  ) +
  theme_minimal(base_size = 14) +
...
ggsave(filename = file.path(files_dir, "peak-binned_lung_ratio_plot_binned-2.png"),
       plot = binned_plot,
       width = 8,
       height = 10,
       dpi = 300,
...
ggsave(filename = file.path(files_dir, "lung_ratio_plot_binned.png"),
       plot = binned_plot,
       width = 8,
       height = 10,
       dpi = 300,
