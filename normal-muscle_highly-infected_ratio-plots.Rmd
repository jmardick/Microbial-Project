...
sample_dir <- gsub("\\\\", "/", r"(C:\Users\Jacob\OneDrive - Baylor College of Medicine\Documents\Manoj\Microbial-Project\Data\Kyana Normal vs 3+ Infected)")
...
isolate_specimen_match <- d %>%
  filter(str_detect(sample_name,
                    regex("1046|BN_1511A063f_09262017",
                          ignore_case = TRUE))) %>%
  mutate(
    patient_id = case_when(
      str_detect(sample_name, "1046") ~ "1046",
      str_detect(sample_name, "BN_1511A063f_09262017") ~ "15-11-A063f",
      TRUE ~ "oops"
    ),
