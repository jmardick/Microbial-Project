...
sample_dir <- gsub("\\\\", "/", r"(C:\Users\Jacob\OneDrive - Baylor College of Medicine\Documents\Manoj\Microbial-Project\Data\Match Normal vs 3+ Infected)")
...
ratio_plots <- imap(ratio_list, function(df, ratio_nm) {
  ...
  df2 <- df %>%
    ...
    mutate(
      species = factor(
        species,
        levels = c(sort(setdiff(unique(species), "Polymicrobial")), "Polymicrobial")
      )
    )

  ## Split by class
  muscle_df  <- df2 %>% filter(class == "Uninfected Tissue")
  species_df <- df2 %>% filter(class != "Uninfected Tissue")

  ## Define colors
  muscle_names <- sort(unique(muscle_df$species))
  species_names <- sort(unique(species_df$species))

  muscle_cols  <- c("magenta4", "red", "blue")[seq_along(muscle_names)]
  ...
  p <- ggplot(df2, aes(x = class, y = ratio)) +
    ...
    geom_jitter(
      ...
      width = 0.15, size = 5.6, alpha = 0.75,
      show.legend = TRUE,
      stroke = 0
    ) +
    scale_fill_manual(
      name = "Uninfected Tissue Type",
    ...
    geom_jitter(
      ...
      width = 0.15, size = 4.6, alpha = 0.75,
      show.legend = TRUE
    ) +
    scale_color_manual(
      ...
      guide = guide_legend(
        override.aes = list(size = 6, alpha = 0.75),
        order = 1
      )
    ) +
    ...
    ## Wilcoxon Test
    {
      if (n_distinct(df2$class) == 2) {
        stat_compare_means(
          data = df2,
          method = "wilcox.test",
          comparisons = list(c("Uninfected Tissue", "Highly Infected Tissue")),
          label = "p.format",
          label.x = 1.5,
    ...
    scale_x_discrete(
      labels = c(
        "Highly Infected Tissue" = "Highly Infected",
        "Uninfected Tissue" = "Uninfected"
      )
    ) +
...
title_B <- bquote(bold("B. Ratios of " * bolditalic("m/z") * " 748.528 : " * bolditalic("m/z") * " 749.534"))

p1 <- ratio_plots[["m/z 747.518 / m/z 748.528"]] + ggtitle(title_A) +
  theme(plot.title = element_text(hjust = 0, size = 18.5))
...
isolate_specimen_match <- d %>%
  filter(str_detect(sample_name,
                    regex("1046|95522A1_Normal_DCIS_19Aug24_a1_SNredo",
                          ignore_case = TRUE))) %>%
  mutate(
    patient_id = case_when(
      str_detect(sample_name, "1046") ~ "1046",
      str_detect(sample_name, "95522A1_Normal_DCIS_19Aug24_a1_SNredo") ~ "95522A1",
      TRUE ~ "oops"
    ),
    sample_type = ifelse(str_detect(sample_name, "1046"),
                         "Infected Breast", "Uninfected Breast"),
    zoom = "Full Spectrum"
  )
...
dd1 <- dd1 %>%
  mutate(
    mz = ifelse(mz == 749.532, 749.534, mz),
    facet_col = paste0(patient_id, " - ", sample_type),
    facet_col = factor(facet_col,
                       levels = c("1046 - Infected Breast",
                                  "95522A1 - Uninfected Breast")),
  ...
  facetted_pos_scales( 
    x = list( 
      zoom == "Full Spectrum" ~ scale_x_continuous( 
        ...
        expand = expansion(mult = c(0, 0), add = c(15, 5)) 
      ), 
      zoom == "Ratio Peaks Region" ~ scale_x_continuous( 
        limits = c(747, 750), 
        breaks = seq(747, 750, by = 1), 
...
``` {r save figure 4}
## Add panel-C title
title_C <- bquote(bold("C. Mass spectra of infected and uninfected breast tissue with their ratio-defining peaks"))
p3 <- p3 + ggtitle(title_C) +
  theme(plot.title.position = "plot",
        plot.title = element_text(hjust = 0, size = 18.5, face = "bold"))
