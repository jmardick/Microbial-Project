...
color_classes <- sort(unique(tsne_df$class))
tsne_colors <- pals::polychrome(length(color_classes)) ## For species classification
#tsne_colors <- pals::glasbey(length(color_classes)) ## For gram classification
names(tsne_colors) <- color_classes

plot6 <- ggplot(tsne_df, aes(x = X, y = Y, fill = class)) +
  geom_point(shape = 21, color = "black", alpha = 1, size = 2.5) +
  scale_fill_manual(values = tsne_colors) +
  theme_minimal() +
  labs(title = "t-SNE of Pure Isolate Top 7 Principal Components", fill = "Species") +
  guides(color = guide_legend(override.aes = list(size = 3))) +   # increase color dot size
  theme(
