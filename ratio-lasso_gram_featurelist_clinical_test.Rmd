...
compute_needed_ratios <- function(x, trained_features) {
  ...
  for (i in seq_along(trained_features)) {
    ...
    if (num %in% colnames(x) && denom %in% colnames(x)) {
      ...
      finite_mask <- !(neg_inf_mask | pos_inf_mask | is.na(ratio))
      valid_vals <- ratio[finite_mask]
      ...
      if (length(valid_vals) >= 1) {
        ...
        # Leave other NA values as NA (do not impute)
        logratios[, i] <- ratio
      } else {
        # Not enough signal — keep column all NA
        logratios[, i] <- NA
      }
    } else {
      # Missing features
