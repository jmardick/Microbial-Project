...
if (peak_alignment_method == "clustering") {
  ...
  save(preprocList, 
       file = file.path(out_dir, files_dir, 
...
trained_features <- rownames(as.matrix(coef(model)))
...
compute_needed_ratios <- function(x, trained_features, min_valid = 10) {
  ...
  for (i in seq_along(trained_features)) {
    ...
    if (num %in% colnames(x) && denom %in% colnames(x)) {
      ...
      valid_vals <- ratio[!(neg_inf_mask | pos_inf_mask)]
      ...
      if (length(valid_vals) >= min_valid) {
        ...
        ratio[is.na(ratio)] <- med
        
        logratios[, i] <- ratio
      } else {
        ## Not enough valid data — leave this ratio column as NA
        logratios[, i] <- NA
      }
    } else {
      ## Missing features — leave this ratio as NA
      logratios[, i] <- NA
    }
  }
...
xall <- compute_needed_ratios(x = xall, trained_features = trained_features)

## Remove any ratio columns that have only NA values
valid_ratios <- colSums(!is.na(xall)) >= 10 # Dependent on min_valid above
xall <- xall[, valid_ratios, drop = FALSE]
trained_features <- colnames(xall)
```
