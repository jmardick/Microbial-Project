...
sample_dir <- gsub("\\\\", "/", r"(C:\Users\Jacob\Box\Eberlin_Lab_BCM\People\MANOJ\Renamed_Gram_positive_Gram_negative_Isolates_May2025\Excel files\Gram-positive\Staphylococcus aureus)")
...
sample_names_df <- sample_names_df %>%
  filter(sample_name %in% rownames(xall)) %>%
  arrange(match(X, rownames(xall)))

set.seed(seed)
test_count <- round(table(sample_names_df)*0.3,0)
test_index <- vector()

for (lbl in levels(yall)){
  idx <- which(yall == lbl & is.na(sample_names_df$patient))
  n_test <- round(length(idx) * (1 - train_fraction))
  if (n_test > 0) {
    test_index <- c(test_index, sample(idx, n_test))
  }
}

sample_names_df$train_test_set <- "train"
sample_names_df$train_test_set[test_index] <- "test"
