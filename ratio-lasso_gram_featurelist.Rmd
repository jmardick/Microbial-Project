...
subtitle: "m/z 700 - 1000 Feature List"
...
sample_dir <- gsub("\\\\", "/", r"(C:\Users\Jacob\Box\Eberlin_Lab_BCM\Projects\Bacterial Identification\data\culture_isolates\MSPen\negative_ion_mode\solvent_MeOH)")
...
save(model, cvmodel, filtered_mz, best_threshold,mass_range, ppm_error, normalization_method, seed, classes, lasso_coef,
     file=file.path(out_dir, files_dir, paste0(format(Sys.time(), "%Y-%m-%d_%H.%M"),"_",proj_name, "_model.RData")))
```

```{r}
save(list = ls(), file=file.path(out_dir, files_dir, paste0(format(Sys.time(), "%Y-%m-%d_%H.%M"),"_",proj_name, "_ALL.RData")))
```
