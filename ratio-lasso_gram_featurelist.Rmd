...
```{r pca}
# Run PCA
allpairs_filtered <- allpairs[, apply(allpairs, 2, function(col) sd(col) != 0)]
pca <- prcomp(allpairs_filtered, center = TRUE, scale. = TRUE)
...
plot4 <- ggplot(pca_df, aes(x = PC1, y = PC2, color = class)) +
  ...
  labs(
    title = "PCA of All Log-Ratio Features by Class", 
...
plot5 <- ggplot(elbow_df[1:36, ], aes(x = seq_along(Variance), y = Variance)) +
  ...
  labs(title = "Log-Ratio Feature PCA Scree Plot",
...
set.seed(42)
tsne_result <- Rtsne(pca_input_unique, dims = 2, perplexity = 50, max_iter = 1000, verbose = TRUE)
...
plot6 <- ggplot(tsne_df_clean, aes(x = X, y = Y, color = class)) +
  geom_point(alpha = 0.5, size = 1.2) +
  theme_minimal() +
  labs(title = "t-SNE of Top 10 Log-Ratio PCs", color = "Class") +
  guides(
