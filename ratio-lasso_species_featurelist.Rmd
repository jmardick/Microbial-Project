...
classes <- gsub("(_|\\s)?[Rr]enamed.*|(_|\\s)?[Ee]xcel.*", "", classes)
...
file_name_list <- lapply(classes_path, function(x) 
  list.files(path = file.path(sample_dir,x), pattern = paste0("*.", sample_file_ext), full.names = TRUE, 
             #recursive = FALSE))
             recursive = TRUE)) ## If files are in sub-directories within class directories
...
file_name_list <- file_name_list[lengths(file_name_list) >= 10]
...
test_count <- round(table(sample_names_df$class)*0.3,0)
test_index <- vector()
for (c in classes){
  test_index <- c(test_index, sample(which(sample_names_df$class == c & is.na(sample_names_df$patient)), size = test_count[c]))
}
...
save(model, cvmodel, filtered_mz, mass_range, ppm_error, normalization_method, seed, classes, classes_path, lasso_coef, peak_alignment_method,
