---
title: "Ratio Lasso - GAS vs GBS Prediction from Pure Samples"
...
keep_idx <- grepl("^(Streptococcus pyogenes|Streptococcus agalactiae)\\b", species)
species_keep <- species[keep_idx]
classes <- ifelse(grepl("^Streptococcus pyogenes\\b", species_keep), "GAS", "GBS")
...
sample_names_df$GAS_GBS <- NA_character_
sample_names_df$GAS_GBS[sample_names_df$sample_name %in% sample_names_list$GAS] <- "GAS"
sample_names_df$GAS_GBS[sample_names_df$sample_name %in% sample_names_list$GBS] <- "GBS"
...
for (cls in unique(sample_names_df$GAS_GBS)) {
  if (is.na(cls)) next

  df_cls <- sample_names_df[has_patient & sample_names_df$GAS_GBS == cls, , drop = FALSE]
...
for (cls in unique(sample_names_df$GAS_GBS)) {
  ...
  idx <- which(no_patient & sample_names_df$GAS_GBS == cls)
