---
title: "Ratio Lasso - Staph vs Strep Prediction from Pure Samples"
...
sample_dir <- gsub("\\\\", "/", r"(C:\Users\Jacob\Box\Eberlin_Lab_BCM\People\MANOJ\Renamed_Gram_positive_Gram_negative_Isolates_May2025\Excel files\Gram-positive)")
...
## Strep vs Staph
species <- vapply(classes, `[`, character(1), 1)
keep_idx <- grepl("^(Staphylococcus|Streptococcus)\\b", species)
species_keep <- species[keep_idx]
classes <- ifelse(grepl("^Staphylococcus\\b", species_keep), "Staphylococcus", "Streptococcus")
classes_path <- split(species_keep, classes)
classes <- names(classes_path)
...
## Ensure only include sample names that exist in this analysis
valid_sample_names <- unlist(sample_names_list, use.names = FALSE)
sample_names_df <- sample_names_df[sample_names_df$sample_name %in% valid_sample_names, ]

## Gram stain classification
sample_names_df$strep_staph <- NA_character_
sample_names_df$strep_staph[sample_names_df$sample_name %in% sample_names_list$Staphylococcus] <- "Staphylococcus"
sample_names_df$strep_staph[sample_names_df$sample_name %in% sample_names_list$Streptococcus] <- "Streptococcus"

set.seed(seed)

sample_names_df$patient <- as.character(sample_names_df$patient)

## logical masks (NOT indices)
has_patient <- !is.na(sample_names_df$patient) & sample_names_df$patient != ""
no_patient  <-  is.na(sample_names_df$patient) | sample_names_df$patient == ""

## 1) patient-safe split among labeled patients (stratified by strep/staph)
train_patients <- character(0)

for (cls in unique(sample_names_df$strep_staph)) {
  if (is.na(cls)) next

  df_cls <- sample_names_df[has_patient & sample_names_df$strep_staph == cls, , drop = FALSE]
  pats <- unique(df_cls$patient)
  pats <- pats[!is.na(pats) & pats != ""]

  if (length(pats) == 0) next

  n_train <- round(length(pats) * train_fraction)
  n_train <- max(1, n_train)
  n_train <- min(n_train, length(pats))

  train_patients <- c(train_patients, sample(pats, n_train))
}

train_patients <- unique(train_patients)

## patient-labeled rows in train
train_index <- which(has_patient & sample_names_df$patient %in% train_patients)

## 2) add some no-patient samples to train, stratified by class
train_index_no_patient <- integer(0)

for (cls in unique(sample_names_df$strep_staph)) {
  if (is.na(cls)) next

  idx <- which(no_patient & sample_names_df$strep_staph == cls)
  if (length(idx) == 0) next

  n_train_np <- round(length(idx) * train_fraction)
  n_train_np <- max(1, n_train_np)
  n_train_np <- min(n_train_np, length(idx))

  train_index_no_patient <- c(train_index_no_patient, sample(idx, n_train_np))
}

train_index <- sort(unique(c(train_index, train_index_no_patient)))
test_index  <- setdiff(seq_len(nrow(sample_names_df)), train_index)

sample_names_df$train_test_set <- "test"
sample_names_df$train_test_set[train_index] <- "train"
otr <- which(sample_names_df$train_test_set == "train")
```
