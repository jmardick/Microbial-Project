...
model_path <- gsub("\\\\", "/", r"(C:\Users\Jacob\OneDrive - Baylor College of Medicine\Documents\Manoj\Microbial-Project\outputs\ratio-lasso_species_featurelist_2026-02-17\2026-02-17_12.13_ratio-lasso_species_featurelist_files\2026-02-17_12.15_ratio-lasso_species_featurelist_ALL.RData)")

## Full path to folder with sample files (excel or csv, or raw Thermo files)
sample_dir <- gsub("\\\\", "/", r"(C:\Users\Jacob\OneDrive - Baylor College of Medicine\Documents\Manoj\Microbial-Project\Data\Match Normal vs 3+ Infected\Highly Infected Tissue)")
...
model_classes <- model_env$classes
mass_range <- model_env$mass_range
peak_alignment_method <- model_env$peak_alignment_method
filtered_mz <- model_env$filtered_mz
ppm_error <- model_env$ppm_error
normalization_method <- model_env$normalization_method
model <- model_env$model
cvmodel <- model_env$cvmodel
```

```{r file extension}
classes_path <- gsub(file.path(sample_dir, "/"), "", list.dirs(sample_dir)[c(-1)], fixed=TRUE)
## Remove polymicrobial samples
classes_path <- classes_path[!grepl("polymicrobial", classes_path, ignore.case = TRUE)]

classes <- strsplit(classes_path, "/")
classes_idx <- classes %in% model_classes
classes <- classes[classes_idx]

classes_path <- lapply(classes, function(x) paste(x, collapse = '/'))

names(classes_path) <- classes[classes_idx]
...
fixed_objects <- list(mass_range = mass_range, file_name_list = file_name_list)
```
...
```{r yall}
all_lvls <- model$classnames # trained classes

## create yall object 
yall <- factor(as.character(sample_names_df$class), levels = all_lvls)
```
...
test_p_class <- factor(all_lvls[test_p_class],levels=all_lvls)
...
test_df <- data.frame(file_name=sample_names, 
                      true_class=as.character(yall), 
                      prediction_class=as.character(test_p_class), 
...
```{r chunk4}
test_recalls <- paste0(format(round(sapply(seq_along(all_lvls),
  function(i) ifelse(sum(test_cm[i,]) > 0, test_cm[i,i]/sum(test_cm[i,])*100, NA_real_)
), 2), nsmall = 2), "%")
...
prob_names <- colnames(test_wrong_preds)[-(1:3)]
align <- paste0("lll", paste0(rep("c", length(prob_names)), collapse = ""))

kable(test_wrong_preds,
      #caption = "Test Set Misclassifications",
      col.names = c("Sample Name", "True Class", "Predicted Class", prob_names),
      row.names = FALSE,
      align = align)  %>%
  ...
  add_header_above(c(" " = 3, "Probability" = length(prob_names)))
```
